...
- **Force Import** — hold grid import at a fixed target level (e.g. topping up the battery from the grid during a cheap tariff window), charging the battery with whatever combination of that import and spare solar it takes. Unlike the other actions here, this isn't a single write: house load and solar shift constantly, so the plugin continuously re-corrects the setpoint against the actual measured grid power on every poll cycle (a proportional servo loop, tuned the same as the reference implementation's own), and stops automatically, early, once the battery settles near zero power flow — evidence the inverter's own SoC Control loop has already reached the cutoff — rather than waiting out the full configured duration. It also **pauses itself** (writes a neutral dispatch but keeps the session active, rather than fully stopping) if the target can't actually be met without draining the battery, PV is clipping against your inverter's AC limit, or the inverter leaves Normal work mode at all (e.g. a grid outage) — and only **resumes** once work mode has been confirmed Normal for a full 10 minutes and the overload has cleared.
- **Dispatch (Advanced)** — the general-purpose action, with a picker for all 8 documented Dispatch modes (Battery only Charges from PV, State of Charge Control, Load Following, Maximise Output, Normal Mode, Optimise Consumption, Maximise Consumption, No Battery Charge). The Power/Cutoff SoC fields only apply in modes 1/2/3/5 — modes 4/6/7 run their own fixed algorithm regardless of what's in either field (confirmed live: 4 discharges the battery to maximise grid feed-in, 6/7 force-charge from grid), not "neutral" as earlier notes here claimed.
